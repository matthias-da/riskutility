...
abstract: >
  ...
  RF proximity), record linkage, and membership inference (DOMIAS, NNAA, singling
  out, linkability) --- alongside a broad suite of utility measures including
...
linkability, and inference --- all of which **riskutility** measures directly.
...
riskutility:::print.ldiversity(l_res)
...
A DCR share above 0.5 (or ratio below 1) indicates memorization. The
**NNDR** (Nearest Neighbor Distance Ratio) provides a complementary view:
for each synthetic record, the ratio of the distance to its nearest
neighbor to its second-nearest neighbor. A ratio near 0 indicates a single
dominant match; near 1 indicates no distinctive match. **IMS** (Identical
...
vulnerable to membership inference attacks. Distance-based metrics should
always be complemented with other risk families.
...
anonymization failure criteria [@stadler2022synthetic; @wp29anonymisation].

**NNAA** (Nearest Neighbor Adversarial Accuracy, @yale2020generation) compares,
for each real record, the label (training vs holdout) of its nearest synthetic
neighbor. The privacy loss is the difference between the share of training
records correctly identified and 0.5 (random chance):
...
**Singling out** and **linkability** operationalize the Article 29 Working
Party's three anonymization criteria:
...
The **energy distance** [@szekely2013energy] provides a multivariate two-sample
test that is sensitive to differences in both location and scale:
...
assessment. A low DCAP score does not prove no attacker can succeed.
