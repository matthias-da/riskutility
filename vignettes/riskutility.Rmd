...
distance-based (Gower distance) or probabilistic (Fellegi-Sunter) linkage
...
dataset. Two methods are implemented: distance-based (Gower distance) and
probabilistic (Fellegi-Sunter, @fellegi1969theory). Three matching modes are
available: independent (many-to-one), bijective (one-to-one via Hungarian
algorithm, @herranz2016gdbrl), and optimal transport (Sinkhorn).
...
Example usage (not evaluated here):
    rl_res <- riskutility::recordLinkage(pair, method = "distance-based")
    print(rl_res)
-->
...
rl <- data.frame(
  ...
  Distance = c("Gower", "Fellegi-Sunter"),
  `Mixed types` = c("Yes", "Yes"),
  Matching = c("All 3", "All 3"),
  check.names = FALSE
)
knitr::kable(rl,
