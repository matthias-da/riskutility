...
author:
  - name: Matthias Templ
    ...
    email: \email{matthias.templ@gmail.com}
  - name: Oscar Thees
...
abstract: >
  ...
  on Real (TSTR). The multivariate Risk-Utility map (\code{rumap()}) jointly
  evaluates multiple risk and utility dimensions and identifies Pareto-optimal
  synthesizers. All functions share a consistent S3 API with \code{print()},
...
distance-based or membership inference methods. In Python, SDMetrics [@patki2016sdv] covers
distance-based metrics and Anonymeter [@giomi2023anonymeter] implements the GDPR
failure criteria, but neither addresses attribution risk; the broader evaluation
libraries synthcity [@qian2023synthcity] and SynthEval [@lautrup2025syntheval]
are also Python-only. No existing *R* package provides a unified
...
knitr::kable(scope, caption = "Package scope at a glance.
             Both = applicable to traditionally anonymized and synthetic data.\\label{tab:scope}")
```
...
Every risk measure in riskutility addresses one or more disclosure threats.
...
distance-based (Gower distance), probabilistic (Fellegi-Sunter), PRAM-aware,
...
@giomi2023anonymeter (singling out, linkability, inference). Neither covers
attribution-based metrics (CAP, RAPID) or traditional privacy models
($k$-anonymity, $l$-diversity).

Table~\ref{tab:software} contrasts the coverage of the main R and Python tools.
...
knitr::kable(comp,
             caption = "Risk/utility measures across R and Python packages.\\label{tab:software}")
```
...
Where definitions overlap, riskutility reproduces the values of established
tools. We illustrate this on the running data using `education` as a categorical
target: synthetic data are generated with \pkg{synthpop}, and the mean
correct-attribution probability (CAP) is computed by both packages.

```{r synthpop-comparison, warning=FALSE}
cmp <- tryCatch({
...
knitr::kable(cmp, row.names = FALSE,
             caption = "Mean CAP from riskutility and synthpop on identical data.\\label{tab:synthpop}")
```

The two implementations agree closely; the small remaining difference reflects
edge conventions (the handling of records with no key match and the baseline
definition; see `?dcap`). The frequency-based measures `kanonymity()` and
...
Figure~\ref{fig:integration} shows how riskutility connects to the wider R
ecosystem.
...
```{r rapid-demo, warning=FALSE, fig.cap="RAPID risk histogram (left) and threshold-sensitivity diagnostic (right).\\label{fig:rapid}"}
...
be close to the baseline. The threshold-sensitivity panel of Figure~\ref{fig:rapid}
...
knitr::kable(models, caption = "RAPID model backends.\\label{tab:rapid-models}")
```
...
dataset. Eight methods are implemented, spanning distance-based (Gower distance),
...
knitr::kable(rl,
             caption = "Record linkage methods. All 3 = independent, bijective, OT.\\label{tab:rl}")
```
...
knitr::kable(comparison,
             caption = "Cross-family comparison: safe vs. risky synthetic data.\\label{tab:rosetta}")
```
...
For the running example---where the synthetic data are drawn independently of
the original---the classifier separates the two samples easily, so the pMSE
falls into the *poor fidelity* range, the correct verdict. Like `propscore()`,
...
estimates are close. Here both measures confirm poor marginal fidelity---the
Hellinger distances are large and the confidence intervals barely overlap.
...
Both the energy distance and the MMD are large, confirming that the joint
distribution of `age` and `income` is not preserved by the independent
synthesizer.
...
The low copula and contingency fidelity is expected: independent synthesis
reproduces marginal frequencies only approximately and destroys the joint
dependence structure entirely.
...
A TSTR-to-TRTR performance ratio well below one indicates that predictive
relationships do not transfer from the synthetic data to real data.
...
knitr::kable(verdicts, caption = "Quick risk screening across three methods.\\label{tab:verdicts}")
```
...
```{r case-rumap-scatter, fig.width=8, fig.height=6, fig.cap="Risk-Utility map of the three synthesizers; the Pareto-optimal front is highlighted.\\label{fig:rumap-scatter}"}
...
```{r case-rumap-heatmap, fig.width=8, fig.height=5, fig.cap="Heatmap of normalized risk and utility measures across the three synthesizers.\\label{fig:rumap-heatmap}"}
...
1. **Comprehensive coverage.** To our knowledge, it is the first R package to
   ...
   comparable evaluation suites (the Python libraries synthcity and SynthEval)
   sit outside R.

2. **Novel implementations.** To our knowledge, it provides the first R
   implementations of RAPID; of the singling-out and linkability attacks
   ...
   density-ratio membership inference; and of eight-method record linkage with
   bijective and optimal-transport matching.
