...
attribute-disclosure measures (CAP, RAPID) estimate; we therefore fold it into
...
utility rather than risk. The standalone \pkg{RAPID} package
[@templ2026rapid] implements the RAPID measure alone; \pkg{riskutility} supersedes
...
essentially never occurs and every TCAP score would collapse to zero. The
three TCAP variants reported in the summary block (certain, matched,
conditional) differ only in numerator and denominator; Section
\ref{sec-validation} defines them and maps them to the \pkg{synthpop}
versions that implement each.
...
TCAP illustrates the same point from the other side: the literature has never
settled on a single denominator for it. \pkg{synthpop} up to version 1.9-2
divided the number of correctly and certainly attributed records by all
original records whose key occurs in the synthetic data; from version 1.9-3 it
adopts the definition of @little2025synthetic, which divides by only the
records matched to a *disclosive* synthetic key group, i.e., one whose
synthetic records all share a single target value [@raab2024practical].
`tcap()` therefore reports the variants side by side rather than privileging
one: `tcap_matched` reproduces the former, `tcap_conditional` the latter, and
`tcap_certain` --- the package default --- is the strictest variant, which
additionally requires the key to determine the target uniquely in the original
data. On any data the three order as certain $\le$ matched $\le$ conditional,
and a `kind` argument selects which of them `print()` and `summary()`
headline. On the running example of Section \ref{sec-risk}:
...
A comparison that pins its estimand this precisely survives upstream
redefinitions; one that compares "TCAP to TCAP" does not. The frequency-based
measures `kanonymity()` and `ldiversity()` are defined identically to their
\pkg{sdcMicro} counterparts.


## ML-Based Risk: RAPID {#sec-rapid}

Risk of Attribute Prediction-Induced Disclosure (RAPID) [@templ2026rapid] takes a
...
**DOMIAS** [@hu2023domias] detects membership by comparing the local density of
the synthetic data around each record against a reference density estimated
from the holdout. Records the generator has overfitted sit where the synthetic
density is disproportionately high relative to that reference:
...
1. **Comprehensive coverage.** To our knowledge, it is the first \proglang{R} package to
   ...
   attribution (CAP), ML-based (RAPID), distance-based, record linkage, and
...
2. **Novel implementations.** To our knowledge, \pkg{riskutility} provides the first
   ...
   record linkage with bijective and optimal-transport matching. RAPID was
   first released in the standalone \pkg{RAPID} package
...
The author thanks Oscar Thees for contributions to parts of the RAPID
implementation, and Roman Müller for contributions to parts of the record
