...
#' #'
#' #' This function calculates and compares by default the empirical cumulative
#' #' distribution functions (ECDF) of a sample (\code{S}) and a population
#' #' (\code{U}) for specified variables. It can handle weighted samples and
#' #' provides options for conditional CDFs, and approximation.
#' #'
#' #' @param X A data frame or data table representing the sample.
#' #' @param Y A data frame or data table representing the population.
#' #' @param variables A character vector specifying the variables for which the
#' #' CDFs are computed. Must be present in both \code{S} and \code{U}.
#' #' @param conditional A character vector specifying conditioning variables,
#' #' or \code{NULL} if no conditioning is desired. Must be of length 1 if
#' #' specified.
#' #' @param weights A character vector of length 1 specifying the weights
#' #' variable in \code{S}, or \code{NULL} if no weights are used.
#' #' @param approx A logical vector of length 2 indicating whether to
#' #' approximate the CDFs in \code{S} and \code{U}, respectively.
#' #' @param n_approx A numeric value specifying the number of points to use
#' #' for the approximation.
#' #' @param bounds A logical value indicating whether to use bounds in the
#' #' CDF calculation.
#' #' @param addplot A logical value indicating whether to add the plot of the
#' #' CDFs.
#' #' @param kindplot A character string specifying the type of plot to generate.
#' #' Default is "compare".
...
#' #'
#' #' @details
#' #' The function calculates the ECDFs for the specified variables in the sample
#' #' and population data sets. If weights are provided, the weighted ECDFs are
#' #' computed. The function supports conditional CDFs, which are computed by
#' #' conditioning on the specified variables.
#' #'
...
#' #' The function also provides an option to add plots of the CDFs.
#' #'
#' #' @import data.table
#' #' @import simPop
#' #'
...
#               n_approx = 10000)
# plot(c3)
# c4 <- compare(eusilc13puf, eusilc13puf_synth,
#               variables = c("age"),
#               weights = "rb050",
#               conditional = "rb090",
#               n_approx = 10000)
# plot(c4)
# c5 <- compare(eusilc13puf, eusilc13puf_synth,
#               variables = c("age"),
#               weights = "rb050",
#               conditional = "db040",
#               n_approx = 10000)
