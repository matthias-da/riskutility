...
linkability.default <- function(X, Y,
  ...
  # Shared input validation, variable intersection, NA removal
  # Note: holdout splitting uses seed + 1L (after aux_cols split uses seed),
  # so we handle aux_cols split between validation and holdout split.
  v <- .validate_pair_inputs(X, Y, holdout = holdout,
                             holdout_fraction = NULL,
                             vars = vars, na.rm = na.rm, seed = NULL,
                             min_vars = 2L)
  X       <- v$X
  Y       <- v$Y
  vars    <- v$vars
